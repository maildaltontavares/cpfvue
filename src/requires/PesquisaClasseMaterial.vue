<template>
  <!-- Modal Pesquisa Classe de material-->
  <div class="modal fade" id="modalPesquisaClasseMaterial" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabelClasseMaterial" aria-hidden="true">
    <div class="modal-dialog  modal-dialog-centered modal-lg">
      <div class="modal-content">
        <div class="modal-header" style="background-color: #003366; color:white; font-weight: bold;">
            <label class="tituloPesquisa fs-5" id="staticBackdropLabelClasseMaterial">Pesquisa Classe de Material</label>
        </div>
        <div class="modal-body" >
          <div class="mb-3">
              <div class="col-md-12 input-group">
                  <input type="text" class="form-control form-control-sm" @blur="apiPesquisaNome('classematerial','nome',nomePesquisa,1)" v-model.lazy="nomePesquisa" placeholder="Informe o nome da Classe de Material">
                  <button type="button" class="btn btn-primary" @click="apiPesquisaNome('classematerial','nome',nomePesquisa,1)"><i class="bi bi-search input-group-append"></i></button>
              </div>
          </div>
              ...
              <thead>
                ...
                <tbody class="detalhePesquisa">
                    ...
                    <tr v-for="c in apiDisplayedDados" :key="c">
                        <th scope="row" class="text-center" style="color: blue; cursor: pointer;" @click="setaPesquisa({ tipo: 'classematerial',obj: c})" data-bs-dismiss="modal">{{ c.id }}</th>
...
<script>
    ...
    $('#modalPesquisaClasseMaterial').on('shown', function () {
        alert("aqui");
    })

    export default {
        name: 'PesquisaClasseMaterial',
        mixins: [ApiMixin],
        data: () => ({
            haErros : false,
            mensagemErro: '',
        ...
        methods:{
            ...
            fecharModal() {
                ...
                this.resultPesquisa = []
                this.pages = []
                $('#modalPesquisaClasseMaterial').modal('hide');
            },
        },
        watch: {
            haErros(newValue) {
